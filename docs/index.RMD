...
A thired aestheitc can be added to scatter plots. Here, color signifies the number of cylinders in each car. Before `ggplot()` is called, Cylinders is created using `library(dplyr)` and the piping operator `%>%`.
...
### Bubble Scatter Plot
TODO(aaron): Create bubble scatter plot
...
Waffle charts can be enhanced by replacing squares qith glyphs. Two important arguments to know are `glyph_size =` and `use_glyph =`. Both are called in the `waffle()` function. 
...
waffle(parts, rows = 5, glyph_size = 1, colors = c("#1696d2", "#fdbf11", "#000000"), legend_pos = "bottom", use_glyph = "medkit") +
  labs(title = "Free Clinics in the D.C.-Maryland-Virginia Area",
