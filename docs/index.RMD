...
unemployment %>%
	...
	ggplot() + 
		...
		geom_text(data = october, mapping = aes(x = time, y = rate, label = as.character(rate)), nudge_x = -0.06) + 
		geom_text(data = august, mapping = aes(x = time, y = rate, label = as.character(rate)), nudge_x = 0.06)
```
...
```{r small multiples 1, message = FALSE, warning = FALSE, fig.height = 9}
# Facet Wrap
diamonds %>%
	group_by()
	ggplot(cut)
